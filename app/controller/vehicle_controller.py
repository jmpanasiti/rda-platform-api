...
class VehicleController():
    ...
    async def upload_auth_idcard(self, vehicle_id: UUID, file: UploadFile):
        try:
            content_file = await file.read()
            file_name = file.filename
            self.vehicle_service.upload_auth_idcard(
                content_file, file_name, vehicle_id)
        except BaseHTTPException as ex:
            raise ex

    async def upload_title(self, vehicle_id: UUID, file: UploadFile):
        try:
            content_file = await file.read()
            file_name = file.filename
            self.vehicle_service.upload_title(
                content_file, file_name, vehicle_id)
        except BaseHTTPException as ex:
            raise ex
    ...
    def download_auth_idcard(self, vehicle_id: UUID, file_name: str):
        try:
            file_path = self.vehicle_service.get_download_path_auth_idcard(
                vehicle_id, file_name)
            return FileResponse(file_path)
        except BaseHTTPException as ex:
            raise ex

    def download_title(self, vehicle_id: UUID, file_name: str):
        try:
            file_path = self.vehicle_service.get_download_path_title(
                vehicle_id, file_name)
            return FileResponse(file_path)
        except BaseHTTPException as ex:
            raise ex
    ...
    def delete_auth_idcard(self, vehicle_id: UUID, file_name: str):
        try:
            self.vehicle_service.delete_auth_idcard(vehicle_id, file_name)
        except BaseHTTPException as ex:
            raise ex

    def delete_title(self, vehicle_id: UUID, file_name: str):
        try:
            self.vehicle_service.delete_title(vehicle_id, file_name)
        except BaseHTTPException as ex:
            raise ex
